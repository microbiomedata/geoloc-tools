...
class GeoEngine():
    ...
    def get_landuse_dates(self, latlon: LATLON) -> []:
        ...
        if not -180 <= lon <= 180:
            raise ValueError("Invalid Longitude: "+str(lon))
        landuseparams = {'latitude': lat,
                      'longitude': lon
                      }
        response = requests.get('https://modis.ornl.gov/rst/api/v1/MCD12Q1/dates', params=landuseparams)
        if response.status_code == 200:
    ...
    def get_landuse(self, latlon: LATLON, startDate, endDate) -> {}:
        ...
        apiStartDate="A"+str(start_date_obj.year)+str(start_date_obj.strftime('%j'))
        ...
        response = requests.get('https://modis.ornl.gov/rst/api/v1/MCD12Q1/subset', params=landuseparams)
        #Retrieve Classification System mapping
        with open(os.path.dirname(__file__)+'/data/ENVO_Landuse_Systems_lookup.csv') as mapper:
            mapping = csv.reader(mapper)
            sytems_map = list(mapping)
        #Retrieve Classification System to ENVO mapping
        with open(os.path.dirname(__file__)+'/data/ENVO_Landuse_lookup.csv') as mapper:
            mapping = csv.reader(mapper)
