...
def test_elevation_death_valley():
    with requests_mock.Mocker() as m:
        m.get("https://maps.googleapis.com/maps/api/elevation/json",
              additional_matcher=lambda request: request.qs == {"locations": ["36.5322649,-116.9325408"], "key": ["test_api_key"]},
            json={
            "results": [{"elevation": -80}]
        })
...
def test_elevation_ocean():
    with requests_mock.Mocker() as m:
        m.get("https://maps.googleapis.com/maps/api/elevation/json",
              additional_matcher=lambda request: request.qs == {"locations": ["0,0"], "key": ["test_api_key"]},
            json={
            "results": [{"elevation": -3492}]
        })
...
def test_elevation_caching(mocker):
    with requests_mock.Mocker() as m:
        ...
        assert spy.call_count == 1
        m.get("https://maps.googleapis.com/maps/api/elevation/json", 
            additional_matcher=lambda request: request.qs == {"locations": ["46,-120"], "key": ["test_api_key"]},
            json={
            "results": [{"elevation": 100}]
        })
